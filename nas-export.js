...
addBtn.addEventListener('click', () => {
    ...
    feld.innerHTML = `
        <legend>Baulast ${count}</legend>
		<div class="row">
    <div class="third">
        <label for="typ_${count}">Art der Baulast</label>
    ...
    <div class="third">
        <label for="bez_${count}">Baulastnummer*</label>
        <input id="bez_${count}" name="bez[]">
    </div>

    <div class="third">
        <label for="date_${count}">Rechtskraft*</label>
        <input id="date_${count}" type="date" name="date[]">
    </div>
</div>
        <label for="umring_${count}">Umringskoordinaten</label>
...
removeBtn.addEventListener('click', () => {
    if (count <= 1) return;
    const ok = confirm(
        `Baulast ${count} wirklich wieder entfernen?`
    );
    if (!ok) return;
